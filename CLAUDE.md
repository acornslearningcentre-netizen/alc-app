...
5. **Merge automatically as soon as CI is green on the PR — no manual "test complete" gate.** (Changed 2026-07-26 — this used to require the user to explicitly say testing was done first; it doesn't anymore.) Because both app services auto-deploy on push to `main` (see *Deployment — Railway* above), merging **is** the deploy trigger — there's no separate deploy step.
...
7. **Transition the ticket to "Done" in Jira** (transition id `41`) once the deploy is confirmed working. Don't mark it Done before that confirmation, even if the merge itself succeeded.
8. **Delete the feature branch** — both locally and on `origin` — once its merge into `main` is complete and confirmed deployed (step 6). Never delete a branch before that confirmation, even if it looks safe. Verify with `git merge-base --is-ancestor origin/<branch> origin/main` before deleting if there's any doubt about whether it actually merged clean.
